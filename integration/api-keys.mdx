...
Modify properties of an existing API key:
...
response = requests.patch(
    ...
    json={"name": "Updated Production Key"}
)

updated_key = response.json()
print(f"✅ API key updated: {updated_key['name']}")
```
...
const response = await fetch(
  "http://localhost:8000/public/v1/api-keys/key_abc123",
  {
    ...
    body: JSON.stringify({ name: "Updated Production Key" })
  }
);

const updatedKey = await response.json();
console.log(`✅ API key updated: ${updatedKey.name}`);
```
</CodeGroup>

<Note>
  You can update the name of an API key, but the actual key value and scopes cannot be changed after creation.
</Note>
