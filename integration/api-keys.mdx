...
Update an existing API key's properties using the PATCH endpoint:
...
<ParamField path="name" type="string">
  A new descriptive name for the API key (1-100 characters)
</ParamField>

<Note>
  The actual key value and scopes cannot be changed after creation. To change scopes, create a new key and revoke the old one.
</Note>
